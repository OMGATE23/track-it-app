...
import { getProcessedTags, monthDayYearFormatDate, numberToTime } from '@/helpers/helper'
...
const TaskInfo = ({task , setShowInfo , setShowUpdateModal} : Props) => {
  ...
  return (
    <div 
      className="fixed top-0 left-0 w-[100vw] h-[100vh]  text-black z-[999999] flex justify-center items-center">
      <div className='md:w-[40%] fade-up z-[9999999] bg-white p-4 rounded-md shadow-md'>
        ...
        <div className=' flex flex-col gap-2   py-4 px-8'>
          ...
          <p className='text-sm'>{numberToTime(task.startTime)} to {numberToTime(task.endTime)}</p>
          <div className='flex flex-wrap items-center gap-2'>
            {
              getProcessedTags(task.tags || []).map(tag => (
                <span key={tag.type+tag.type} className={
                  `rounded-full px-3 py-1 ${tag.background} text-white`
                }>
                  {tag.tag}
                </span>
              ))
