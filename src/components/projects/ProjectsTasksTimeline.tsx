...
const ProjectsTasksTimeline = ({ tasks }: Props) => {
  return (
    <div className={` w-fit mx-auto fade-in-pop h-fit`}>
      ...
      {tasks.map((task, i) => (
        ...
        >
          <div className="px-4 py-6 w-[120px] text-right relative">
            ...
            <div
              className={`size-4 bg-zinc-700 rounded-[50%] absolute top-[50%] translate-y-[-50%] left-[100%] translate-x-[-50%]`}
            />
          </div>
          <div
      ...
      {tasks.length === 0 && (
        <p className="text-zinc-500">No tasks done under this project ...yet</p>
      )}
    </div>
  );
};
