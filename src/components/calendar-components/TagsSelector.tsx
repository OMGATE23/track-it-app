...
const TagsSelector = ({ selectedTags, setSelectedTags }: Props) => {
  ...
  return (
    <div ref={componentRef} className="relative w-[80%] flex items-start gap-2">
      ...
      <div className="flex w-[80%] md:w-[60%]  overflow-x-auto pb-2 items-center gap-2">
      ...
      {openTagsDisplay && (
        <div className="px-8 py-6 mt-2 absolute top-8 outline outline-1 outline-zinc-200 rounded-md z-[20] bg-white flex flex-col mx-auto gap-6 h-[450px] overflow-y-scroll">
          {Object.keys(TAGS as Tags)
            .filter((tagType) => tagType !== "status" && tagType !== "priority")
            .map((tagType) => (
              <div className="flex flex-col gap-4" key={tagType}>
                <p className="font-bold border-b capitalize border-zinc-100">
                  {tagType}
                </p>
                <div className="flex flex-col gap-4">
                  {(TAGS[tagType] as Tag[]).map((tag) => (
                    ...
                    >
                      <input
                        checked={selectedTags.some((t) => t.tag === tag.tag)}
                        onChange={(e) => {
                      ...
                      />
                      <span
                        className={`${tag.background} rounded-full text-white outline outline-1 py-1 px-3`}
                      >
                        {tag.tag}
                      </span>
                    </label>
