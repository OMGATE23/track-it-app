...
function TaskTimer(): JSX.Element {
  ...
  useEffect(() => {
    if (isRunning) {
      timerRef.current = setInterval(() => {
        setCurrentTime(new Date(Date.now() + 15 * 60 * 1000));
      }, 1000);
    } else {
  ...
  const handleStart = () => {
    ...
    setCurrentTime(new Date(Date.now() + 15 * 60 * 60 * 1000));
    setIsRunning(true);
  };
  ...
  const handleDone = () => {
    setIsRunning(false);
    if (
      calculateMinutesPassed(currentTime!) -
        calculateMinutesPassed(startTime!) >=
      15
    ) {
  ...
  return (
    <div className="w-full flex flex-col md:flex-row md:items-center md:justify-between gap-4 px-6 py-4 rounded-md shadow-sm outline outline-1 outline-zinc-200">
      <input
        type="text"
        value={title}
        disabled={isRunning}
      ...
      />
      <div className="flex flex-col md:flex-row md:items-center  gap-4 md:gap-8 z-[9999]">
          ...
          {!isRunning ? (
            ...
            >
              <Play
                stroke="black"
                fill="black"
                className={`size-7 transition-all duration-100 ${
                  title && "hover:fill-blue-700 hover:stroke-blue-700"
                }`}
              />
            </button>
          ) : (
